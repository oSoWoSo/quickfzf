...
#### show an OS ISO download URL `<os> <release> <edition>`
`quickget --show-iso-url fedora 38 Silverblue` or `quickget -u alpine latest`
#### Test if an OS ISO is available `<os> <release> <edition>`
`quickget --test-iso-url nixos 23.05 plasma5` or `quickget -t alpine latest`
#### Show link to distro homepage `<os>`
`quickget --show-distro-homepage debian` or `quickget -s alpine`
#### Open an OS distribution homepage in a browser `<os>`
`quickget --open-distro-homepage  ubuntu-mate` or `quickget -o alpine`
#### Download OS image file into current directory, without creating VM `<os> <release> <edition>`
`quickget --download-iso elementary 7.1` or `quickget -d alpine latest`
